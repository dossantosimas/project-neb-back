...
export class PaymentsController {
  ...
  @ApiOperation({
    summary: 'Obtener todos los pagos o filtrar por usuario, mes y año',
  })
  @ApiQuery({
    name: 'userId',
    required: false,
  ...
  findAll(
    ...
    if (monthNum !== undefined && yearNum !== undefined) {
      // Si también se proporciona userId, filtrar por todo
      if (userId) {
        return this.paymentsService.findByMonthAndYearAndUser(
          monthNum,
          yearNum,
          parseInt(userId, 10),
        );
    ...
    if (monthNum !== undefined) {
      if (userId) {
        return this.paymentsService.findByMonthAndUser(
          monthNum,
          parseInt(userId, 10),
    ...
    if (yearNum !== undefined) {
      if (userId) {
        return this.paymentsService.findByYearAndUser(
          yearNum,
          parseInt(userId, 10),
        );
      }
    ...
    // Si solo se proporciona userId, filtrar por usuario
    if (userId) {
      return this.paymentsService.findByUser(parseInt(userId, 10));
    }
